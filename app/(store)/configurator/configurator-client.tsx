...
import { Upload, Package, MapPin, ShoppingBag, ChevronRight, ChevronLeft, Check, X, Loader2, RotateCw } from 'lucide-react'
...
interface DragState {
  ...
  startY: number
  startAngle: number
  mode: 'draw' | 'move' | 'resize-se' | 'resize-sw' | 'resize-ne' | 'resize-nw' | 'rotate' | null
  ...
  if (products.length === 0) {
    return (
      <div className="text-center py-16 text-neutral-400">
        ...
        <p>Er zijn nog geen producten beschikbaar voor eigen ontwerp.</p>
      </div>
    )
  }
...
// ── Step 3 – Plaatsing op het product ────────────────────────────────────────
...
function StepLocation({
  ...
  const dragRef = useRef<DragState>({ active: false, startX: 0, startY: 0, startAngle: 0, mode: null, initPos: null })
  ...
  const clamp = useCallback((pos: EngravingPosition): EngravingPosition => {
    ...
    return { x, y, width, height, rotation: pos.rotation ?? 0 }
  }, [area])
  ...
  const applyMove = useCallback((clientX: number, clientY: number) => {
    ...
    if (drag.mode === 'draw') {
      ...
      next = clamp({ ...init, x: init.x + init.width - w, y: init.y, width: w, height: Math.max(5, init.height + dy) })
    } else if (drag.mode === 'resize-ne') {
      const h = Math.max(5, init.height - dy)
      next = clamp({ ...init, x: init.x, y: init.y + init.height - h, width: Math.max(5, init.width + dx), height: h })
    } else if (drag.mode === 'rotate') {
      // Calculate angle from center of box to current pointer
      const centerX = init.x + init.width / 2
      const centerY = init.y + init.height / 2
      const currentAngle = Math.atan2(py - centerY, px - centerX) * (180 / Math.PI)
      const deltaAngle = currentAngle - drag.startAngle
      const newRotation = (((init.rotation ?? 0) + deltaAngle) % 360 + 360) % 360
      onPosition({ ...init, rotation: newRotation })
      return
    } else {
      // resize-nw
      const w = Math.max(5, init.width - dx)
      const h = Math.max(5, init.height - dy)
      next = clamp({ ...init, x: init.x + init.width - w, y: init.y + init.height - h, width: w, height: h })
    }
    onPosition(next)
  ...
  const startDrag = useCallback((
    ...
    // For rotate mode: compute the starting angle from center of box to pointer
    let startAngle = 0
    if (mode === 'rotate' && currentPosition) {
    ...
    dragRef.current = { active: true, startX: px, startY: py, startAngle, mode, initPos: currentPosition }
    setIsDragging(true)
  }, [toPercent])
  ...
  const onContainerPointerDown = useCallback((e: React.PointerEvent) => {
    ...
    onPosition(clamp({ x: px - 5, y: py - 5, width: 10, height: 10, rotation: 0 }))
    startDrag(e, 'draw', { x: px, y: py, width: 0, height: 0, rotation: 0 })
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <p className="text-sm text-neutral-500 text-center">
        ...
        <span className="font-semibold text-amber-700">gele gebied</span>{' '}
        om de plaatsing te bepalen. Versleep de hoeken om het formaat aan te passen.
        Sleep het <span className="font-semibold text-amber-700">rondje bovenaan</span> om te draaien.
      </p>
      ...
      >
        ...
        {position && (
          ...
          >
            ...
            <div
              className="absolute inset-0 border-2 border-amber-500 rounded opacity-75 overflow-hidden"
              data-handle="move"
            ...
            {/* Rotate handle — boven het midden van de box */}
            <div
              data-handle="rotate"
              ...
              onPointerDown={(e) => startDrag(e, 'rotate', position)}
            >
              <div className="w-6 h-6 bg-white border-2 border-amber-500 rounded-full shadow-md flex items-center justify-center">
      ...
      {position && (
        <div className="text-xs text-neutral-400 text-center flex items-center justify-center gap-3 flex-wrap">
          <span>Positie: {Math.round(position.x)}% × {Math.round(position.y)}%</span>
          <span>|</span>
          <span>Grootte: {Math.round(position.width)}% × {Math.round(position.height)}%</span>
          <span>|</span>
          <span className="flex items-center gap-1">
            <RotateCw className="h-3 w-3" />
            {Math.round(position.rotation ?? 0)}°
          </span>
        </div>
      )}
    </div>
    ...
    addItem({
      ...
      personalizations: [
        ...
        },
        {
          ...
          fieldLabel: 'Plaatsing op product',
          fieldType: 'text',
          value: JSON.stringify(position),
          displayValue: `X: ${Math.round(position.x)}%, Y: ${Math.round(position.y)}% — ${Math.round(position.width)}% × ${Math.round(position.height)}%${position.rotation ? ` — ${Math.round(position.rotation)}°` : ''}`,
        },
      ],
    })
    ...
    <div className="max-w-xl mx-auto space-y-6">
      ...
      <div className="bg-amber-50 rounded-2xl p-5 border border-amber-100 space-y-4">
        ...
        <div className="flex gap-4 items-start">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-neutral-500 mt-1">
              Eigen ontwerp op {Math.round(position.width)}% &times; {Math.round(position.height)}% van het product
            </p>
          </div>
        </div>
        ...
        <div className="border-t border-amber-100 pt-3 text-sm text-neutral-600 space-y-1">
          <p><span className="font-semibold">Plaatsing:</span> X: {Math.round(position.x)}%, Y: {Math.round(position.y)}%</p>
          <p><span className="font-semibold">Afmeting:</span> {Math.round(position.width)}% × {Math.round(position.height)}%</p>
          {(position.rotation ?? 0) > 0 && (
            <p><span className="font-semibold">Rotatie:</span> {Math.round(position.rotation)}°</p>
          )}
        </div>
      </div>
...
export default function ConfiguratorClient() {
  ...
  const canNext = [
    ...
    !!product,
    !!position,
    false, // step 3 doesn't have a next (submit goes to cart)
  ][step]
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50/40 to-white py-12 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-extrabold text-neutral-800 tracking-tight">Zet jouw eigen ontwerp op een product</h1>
          <p className="text-neutral-500 mt-2 text-base md:text-lg">
            Upload je tekening en maak in vier stappen een uniek cadeau.
          </p>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-neutral-100 shadow-xl p-6 md:p-10">
          <h2 className="text-lg font-bold text-neutral-700 mb-6 flex items-center gap-2">
            ...
            {['Upload je tekening', 'Kies een product', 'Kies de plaatsing', 'Bestelling plaatsen'][step]}
          </h2>
